 ...
 * Responde con análisis del taller/negocio usando IA + SQL seguro.
 */
export const chat = async (req: Request, res: Response) => {
    try {
        ...
        // Cargar historial previo de la conversación para soporte de preguntas encadenadas
        let existing: aiChatService.ChatMessage[] = [];
        try {
            existing = await aiChatService.loadChatSession(userId);
        } catch {
            // Silencioso
        }

        const result = await aiChatService.processAiQuestion(trimmed, userId, existing.slice(-6));
